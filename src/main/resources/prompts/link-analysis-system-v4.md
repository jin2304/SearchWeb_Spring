...
- **1순위 (유형 매칭 - 최우선)**: 사이트 도메인이나 브랜드 명칭(Inflearn, Reddit 등)을 통해 파악된 '서비스 유형'과 일치하는 기존 폴더(예: 강의, 커뮤니티, 유틸리티)가 있다면 무조건 해당 폴더를 선택.
...
- **4순위 (미분류 - 최후의 수단)**: 페이지 제목조차 없고 분석이 아예 불가능한 경우에만 선택. **제목이나 태그가 존재한다면 절대로 선택하지 말 것.**
